...
#if defined(NDEBUG)
# error "Banditcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2611d968a22f5324706442c5695541d922644468b31ba4638c2d73bb4dfc2ac7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Banditcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Banditcoin Signed Message:\n";
...
{
    // Banditcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Banditcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Banditcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Banditcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Banditcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Banditcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa9bce4dbe21583c1b636ac669c49af9b0ee111b7b38b82ba8b7dd53ddee9c8df");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Mach dich frei von den Koenigen die das Leben der Bauern beschoenigen";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047d5797daf2de2194cf6d7ea45b7ae4b363d31696d6ab6838f993940e108e7dc76f70a0ade02c66405253f4165b23b5fe69a5e5750aac30f57bf0c35f5e65d327") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527372247;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084583563;

        if (fTestNet)
        {
            block.nTime    = 1527372188;
            block.nNonce   = 385927030;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x4f5ddaa4fa0307944ec23408041cd79218acf29359796ba46748013048c8deba"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Banditcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BanditcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BanditcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BanditcoinMiner(CWallet *pwallet)
{
    printf("BanditcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("banditcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BanditcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BanditcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BanditcoinMiner, pwallet));
}
